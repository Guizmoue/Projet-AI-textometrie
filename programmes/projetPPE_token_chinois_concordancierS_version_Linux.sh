...
do
    ...
    then
        ...
        then
            grep -Po -i "(\w+\W+){0,5}(AI|artificial intelligence)(\W+\w+){0,5}" $TEXTFILE | sed -E -r "s/(.*)(AI|ai|artificial intelligence|Artificial Intelligence|Artificial intelligence|Ai)(.*)/<tr><td>\1<\/td><td>\2<\/td><td>\3<\/td><\/tr>/" >> "../concordances/${lang}-${lineno}.html"
        fi
        if [ "${lang}" = "fr" ] # concordancier français
        then
            grep -Po -i "(\w+\W+){0,5}(IA|intelligence artificielle)(\W+\w+){0,5}" $TEXTFILE | sed -E -r "s/(.*)(IA|intelligence artificielle|ia|Intelligence Artificielle|Intelligence artificielle|Ia)(.*)/<tr><td>\1<\/td><td>\2<\/td><td>\3<\/td><\/tr>/" >> "../concordances/${lang}-${lineno}.html"
        fi
